...
public class UltraRefreshListView extends ListView implements PtrHandler,AbsListView.OnScrollListener{
    ...
    //是否有footView，默认有,在onScroll添加footView的时候就要进行该属性判断，可以通过removeFootIfNeed去掉底部布局
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        //加载更多的判断
        if(totalItemCount>1&&!isLoadData&&totalItemCount==firstVisibleItem+visibleItemCount&&hasFootView){
            isRefresh =false;
            isLoadData = true;
