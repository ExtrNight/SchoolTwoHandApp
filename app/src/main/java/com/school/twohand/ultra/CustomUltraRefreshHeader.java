package com.school.twohand.ultra;
...
public class CustomUltraRefreshHeader extends RelativeLayout implements PtrUIHandler {
    ...
    /**
     *  在动态改变现实视图时，需要注意的便是onUIPositionChange，
     *  我们只有在手指触摸且status为PtrFrameLayout.PTR_STATUS_PREPARE才修改我们的属性，
     *  其他时候不能修改。因为该方法会一直回调
     */
    @Override
    public void onUIPositionChange(PtrFrameLayout frame, boolean isUnderTouch, byte status, PtrIndicator ptrIndicator) {
        ...
        /* if (currentPos < mOffsetToRefresh && lastPos >= mOffsetToRefresh) {
            if (isUnderTouch && status == PtrFrameLayout.PTR_STATUS_PREPARE) {*//*
        Log.i("info","isUnderTouch"+isUnderTouch+"headHeight: "+headerHeight+" lastPosY "+lastPosY+" offsetToRefresh "+offsetToRefresh
        +" offsetY "+offsetY+" currentPosY "+currentPosY);*/
    }
